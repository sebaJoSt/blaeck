...
void setup()
{
  ...
  device.addStateChannel(F("A_bool"), &aBool).withDeviceClass(F("problem"));
  device.addStateChannel(F("A_byte"), &aByte);
  device.addStateChannel(F("A_short"), &aShort);
  device.addStateChannel(F("A_ushort"), &aUShort);
  device.addStateChannel(F("A_int"), &aInt);
  device.addStateChannel(F("A_uint"), &aUInt);
  device.addStateChannel(F("A_long"), &aLong);
  device.addStateChannel(F("A_ulong"), &aULong);
  device.addStateChannel(F("A_float"), &aFloat).withUnit(F("V")).withDisplayPrecision(3);
  device.addStateChannel(F("A_double"), &aDouble).withUnit(F("V"));
  device.addStateChannel(F("A_text"), aText);

  // ---- B: getter, one per type ----
  device.addStateChannel(F("B_bool"), BlaeckBool).withStateValue(gGetBool);
  device.addStateChannel(F("B_byte"), BlaeckByte).withStateValue(gGetByte);
  device.addStateChannel(F("B_short"), BlaeckShort).withStateValue(gGetShort);
  device.addStateChannel(F("B_ushort"), BlaeckUShort).withStateValue(gGetUShort);
  device.addStateChannel(F("B_int"), BlaeckInt).withStateValue(gGetInt);
  device.addStateChannel(F("B_uint"), BlaeckUInt).withStateValue(gGetUInt);
  device.addStateChannel(F("B_long"), BlaeckLong).withStateValue(gGetLong);
  device.addStateChannel(F("B_ulong"), BlaeckULong).withStateValue(gGetULong);
  device.addStateChannel(F("B_float"), BlaeckFloat).withStateValue(gGetFloat).withDisplayPrecision(4);
  device.addStateChannel(F("B_double"), BlaeckDouble).withStateValue(gGetDouble);
  device.addStateChannel(F("B_text"), BlaeckText).withStateText(gGetText);
  device.addStateChannel(F("B_overlong"), BlaeckText).withStateText(gGetOverlong).diagnostic();

  // ---- C: tag, one per type ----
  device.addStateChannel(F("C_bool"), BlaeckBool);
  device.addStateChannel(F("C_byte"), BlaeckByte);
  device.addStateChannel(F("C_short"), BlaeckShort);
  device.addStateChannel(F("C_ushort"), BlaeckUShort);
  device.addStateChannel(F("C_int"), BlaeckInt);
  device.addStateChannel(F("C_uint"), BlaeckUInt);
  device.addStateChannel(F("C_long"), BlaeckLong);
  device.addStateChannel(F("C_ulong"), BlaeckULong);
  device.addStateChannel(F("C_float"), BlaeckFloat);
  device.addStateChannel(F("C_double"), BlaeckDouble);
  device.addStateChannel(F("C_text"), BlaeckText);
  ...
  device.addStateChannel(F("C_never"), BlaeckLong).diagnostic();
  ...
  device.addStateChannel(F("C_once"), BlaeckLong).diagnostic();
...
void RunLocalChecks()
{
  ...
  aByte = 0; device.writeState(F("A_byte"), 21);
  Check(F("int literal -> byte channel"), aByte == 21);

  aFloat = 0.0f; device.writeState(F("A_float"), 21);
  Check(F("int literal -> float channel"), aFloat == 21.0f);

  aDouble = 0.0; device.writeState(F("A_double"), 21);
  Check(F("int literal -> double channel"), aDouble == 21.0);

  aLong = 0; device.writeState(F("A_long"), 21);
  Check(F("int literal -> long channel"), aLong == 21);

  aBool = false; device.writeState(F("A_bool"), 21);
  Check(F("nonzero -> bool channel is true"), aBool == true);

  aBool = true; device.writeState(F("A_bool"), 0);
...
void PushEverything()
{
  ...
  for (byte i = 0; i < sizeof(readers) / sizeof(readers[0]); i++)
    device.writeState(readers[i]);

  // C has no variable and no getter, so each is handed a value - and the extremes are the ones
  // a width bug shows up in first.
  device.writeState(F("C_bool"), gFault);
  device.writeState(F("C_byte"), (byte)UCHAR_MAX);
  device.writeState(F("C_short"), (short)SHRT_MIN);
  device.writeState(F("C_ushort"), (unsigned short)USHRT_MAX);
  device.writeState(F("C_int"), INT_MAX);
  device.writeState(F("C_uint"), UINT_MAX);
  device.writeState(F("C_long"), LONG_MIN);
  device.writeState(F("C_ulong"), ULONG_MAX);
  device.writeState(F("C_float"), (float)-FLT_MAX);
  device.writeState(F("C_double"), (double)DBL_MAX);

  char line[48];
  snprintf(line, sizeof(line), "tick %lu, %u clients", Uptime, gClients);
  device.writeState(F("C_text"), line);
}
...
void WriteOnce()
{
  ...
  written = true;
  device.writeState(F("C_once"), (long)ARM_C_ONCE_VALUE);
