...
void setup()
{
  ...
  device.onNumberCommand("N_bare", onNumber)
      .withRange(0.0f, 100.0f, 1.0f);

  device.onNumberCommand("N_unit", onNumber)
      .withRange(-20.0f, 40.0f, 0.5f)
      .withUnit(F("V"));

  // Step 0 means "say nothing", not "zero resolution" - the key must not appear at all.
  device.onNumberCommand("N_step_unset", onNumber)
      .withRange(0.0f, 10.0f, 0.0f);

  device.onNumberCommand("N_mode_box", onNumber)
      .withRange(0.0f, 10.0f, 1.0f)
      .withMode(BLAECK_NUMBER_MODE_BOX);

  device.onNumberCommand("N_mode_slider", onNumber)
      .withRange(0.0f, 10.0f, 1.0f)
      .withMode(BLAECK_NUMBER_MODE_SLIDER);

  device.onNumberCommand("N_deviceclass", onNumber)
  ...
  device.onNumberCommand("N_tiny_step", onNumber)
      .withRange(0.0f, 1.0f, 0.0001f);

  device.onNumberCommand("N_state_own", onNumber)
      .withRange(0.0f, 5.0f, 0.1f)
      .withOwnState(F("N_state_own_state"), &nStateOwn);

  device.onNumberCommand("N_state_signal", onNumber)
      .withRange(0.0f, 1000.0f, 1.0f)
      .withStateFromSignal(F("Uptime"));

  // ---- S: switches ---------------------------------------------------------------------------
  device.onSwitchCommand("S_bare", onSwitch);

  device.onSwitchCommand("S_deviceclass", onSwitch)
      .withDeviceClass(F("outlet"));

  device.onSwitchCommand("S_state", onSwitch)
      .withOwnState(F("S_state_state"), &sState);

  // ---- L: selects, which take their state two different ways --------------------------------
  device.onSelectCommand("L_bare", onSelect)
  ...
  device.onSelectCommand("L_state_text", onSelect)
  ...
  device.onSelectCommand("L_state_numeric", onSelect)
  ...
  device.onButtonCommand("B_bare", onButton);

  device.onButtonCommand("B_press_payload", onButton)
      .withPressPayload(F("PING"));

  device.onButtonCommand("B_deviceclass", onButton)
      .withDeviceClass(F("restart"));

  // ---- T: text --------------------------------------------------------------------------------
  device.onTextCommand("T_bare", onText)
      .withMaxLength(32);

  device.onTextCommand("T_password", onText)
      .withMaxLength(16)
  ...
  device.onTextCommand("T_maxlen_toobig", onText)
      .withMaxLength(300);
  ...
  device.onTextCommand("T_no_maxlen", onText);

  // ---- how a host files it, isolated on their own controls so nothing else masks them --------
  device.onSwitchCommand("Named", onSwitch)
      .withDisplayName(F("A Friendly Command"));

  device.onSwitchCommand("Iconed", onSwitch)
      .withIcon(F("mdi:tune"));

  device.onSwitchCommand("Config_category", onSwitch)
      .config();

  device.onSwitchCommand("Diag_category", onSwitch)
      .diagnostic();

  device.onSwitchCommand("Hidden", onSwitch)
      .disabledByDefault();

  device.onButtonCommand("Reboot", onReboot)
      .withDeviceClass(F("restart"))
      .diagnostic();
