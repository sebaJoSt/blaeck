...
void setup()
{
  ...
  device.onCommand("E_badtype", onFireBadType);
  device.onCommand("E_badchannel", onFireBadChannel);
  device.onCommand("E_disabled", onFireDisabled);
  device.onCommand("E_case", onFireCase);
...
{
  (void)p; (void)n;
  device.writeEvent(F("Activity"), F("idle"));
  Fired(c);
}
...
{
  (void)p; (void)n;
  device.writeEvent(F("Faults"), F("overheat"));
  Fired(c);
}
...
{
  (void)p; (void)n;
  device.writeEvent(F("Activity"), F("exploded"));
  Fired(c);
}
...
{
  (void)p; (void)n;
  device.writeEvent(F("Nope"), F("idle"));
  Fired(c);
}
...
{
  (void)p; (void)n;
  device.writeEvent(F("Rare"), F("seen"));
  Fired(c);
}
