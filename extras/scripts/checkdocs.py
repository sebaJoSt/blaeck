...
Usage:
  checkdocs.py src/Blaeck.h -- -Iextras/tests/host
  checkdocs.py src/Blaeck.h --show tick -- -Iextras/tests/host
  checkdocs.py src/Blaeck.h --extract -- -Iextras/tests/host
...
def declared_names(tu):
    ...
    """
    return {c.spelling for c in tu.cursor.walk_preorder() if c.kind in CALLABLE}
...
def source_of(c, paths):
    """Which of the given headers a declaration is in, spelled as it was given, or None."""
    if not c.location.file:
        return None
    here = _norm(c.location.file.name)
    for p in paths:
        if _norm(p) == here:
            return p
    return None
...
def public_api(tu, paths):
    for c in tu.cursor.walk_preorder():
        ...
        if c.spelling.startswith("_"):
            continue
        if c.kind == ci.CursorKind.CXX_METHOD and c.is_deleted_method():
            continue
        if c.kind == ci.CursorKind.FIELD_DECL and not owner(c).startswith(FIELD_OWNER):
            continue
        yield c

def show(tu, paths, needle):
    ...
    for c in public_api(tu, paths):
        ...
        print("%s   [%s:%d]" % (what, source_of(c, paths), c.location.line))
        raw = c.raw_comment
        if not raw:
...
def code_block_lines(raw):
    ...
    the header, so a compiler error names src/Blaeck.h and not a generated
...
def extract(tu, paths, dest):
    ...
    out = ['// GENERATED by extras/scripts/checkdocs.py --extract.',
           ...
           '//',
           '// Every @code block from the headers - the snippets an editor shows when',
    ...
    for c in public_api(tu, paths):
        ...
        if any(DEFINITION.match(line.strip().split("{", 1)[0].strip())
               for line in lines):
            ...
            for line in lines:
                line = re.sub(r"\bvoid\s+(loop|setup)\s*\(\s*\)",
                              lambda m: "void _doc%s%d()" % (m[1].capitalize(), n), line)
...
def main(argv):
    ...
    if not groups:
        print("No public API found in " + path, file=sys.stderr)
        return 1

    def at(c):
        return "%s:%d" % (os.path.basename(source_of(c, paths)), c.location.line)
    ...
    if bare:
        ...
        for (cls, name), members in bare:
            ...
            print("  %-24s  %-44s %d overload(s)" % (at(members[0]), where, len(members)))
    if partial:
        print()
    ...
    if jammed:
        ...
        for (cls, name), members in jammed:
            print("  %-24s  %s" % (at(members[0]), "%s::%s" % (cls, name) if cls else name))
    ...
    for (cls, name), members in no_block:
        where = "%s::%s" % (cls, name) if cls else name
        print("  %-24s  %s" % (at(members[0]), where))
