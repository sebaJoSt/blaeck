...
press a button. You write a function, register it under a name, and Blaeck calls it when
that name arrives.
...
void setup()
{
  ...
  device.onNumberCommand("SET_FREQ", onSetFreq).withRange(0.0, 2.0, 0.01);
}
...
- `device.tick()` in `loop()` is what reads the serial port and calls the handler. A sketch
  that answers commands but logs nothing calls `device.read()` instead.
...
Blaeck checks a value before your handler runs. A number outside the range, a position
...
```cpp
device.onButtonCommand("ACTIVATE_ALL", onActivateRange)
...
`writeCommandState()` here - the signal reports the value, so `device.write()` sends it. Leave
