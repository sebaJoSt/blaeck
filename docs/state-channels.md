...
void setup()
{
  ...
  device.addStateChannel(F("Uptime"), &uptime).withUnit(F("s"));
  device.addStateChannel(F("Status"), status).withIcon(F("mdi:message-text"));
}
...
device.writeState(F("Temperature"), 20);      // sends 20.0
```
...
device.addStateChannel(F("Status"), BlaeckText).withStateText(statusText);
```
