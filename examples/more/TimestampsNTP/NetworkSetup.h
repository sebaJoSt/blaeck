/*
  ...
  then networkBegin(SERVER_PORT) in setup() before device.begin(), and networkLoop() in
  ...
  NetworkSetup::Server names the board's server type. Construct one in the sketch, call
  server.begin() after bringing the network up, then pass it to device.begin(server).
